...
/* -------- 测试数据参数 -------- */
#define MAX_MEM_SIZE (128 * GB)   // 内存池管理的每个内存块大小
#define MEM_SIZE (64 * MB)     // 内存池管理的每个内存块大小
#define DATA_N (100000)           // 数据条数
#define DATA_MAX_SIZE (256 * KB)  // 每条数据最大尺寸
...
#ifdef _Z_MEMORYPOOL_H_
    thread pid1(test_fn, mp);
    // pthread_create(&pid2, &attr, test_fn, mp);
    // pthread_create(&pid3, &attr, test_fn, mp);
#else
    thread pid1(test_fn, nullptr);
    // pthread_create(&pid2, &attr, test_fn, NULL);
    // pthread_create(&pid3, &attr, test_fn, NULL);
#endif
    pid1.join();
