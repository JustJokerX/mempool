...
int mp_cmp_function(const void* first, const void* second) {
    // const mem_size_t* f = (const int*) first;
    // const mem_size_t* s = (const int*) second;
...
//#define MP_ALIGN_SIZE(_n) (next_pow2(_n))
...
static _MP_Memory* extend_memory_list(MemoryPool* mp, mem_size_t new_mem_sz) {
    ...
    mm->next = mp->mlist;
    mp->mlist = mm;
    mp->tree = bostree_new(mp_cmp_function, NULL);
    _free = mm->free_list;
    bostree_insert(mp->tree, _free->alloc_mem, _free, &_free->avl_node);
    _free->avl_added = 1;
    return mm;
}
...
FIND_FREE_CHUNK:
    ...
    while (mm) {
        ...
        BOSNode* node = bostree_lookup(mp->tree, total_needed_size);
        if (node) {
            _free = node->data;
            if (_free->alloc_mem >= total_needed_size) {
                ...
                if (_not_free->avl_added == 1) {
                    bostree_remove(mp->tree, &_not_free->avl_node);
...
int MemoryPoolFree(MemoryPool* mp, void* p) {
    ...
    if (ck->avl_added == 1) {
        bostree_remove(mp->tree, &ck->avl_node);
        ck->avl_added = 0;
    }
    ck->is_free = 1;
